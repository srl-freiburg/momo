...
def callback(data):
    ...
    for a in data.agent_states:
        v = np.array([a.position.x, a.position.y, a.velocity.x,
                     a.velocity.y], dtype=np.float32)
    ...
    if distance > parms.goal_threshold:
        set_agent_state(parms.target_id,
                        robot[0], robot[1], path[LOOKAHEAD][2], path[LOOKAHEAD][3])
    else:
...
def listener():
    ...
    rospy.Subscriber("static_obstacles", GridCells,
                     obstacle_callback, queue_size=1)
